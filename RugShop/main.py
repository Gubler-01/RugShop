...
class ListarProductosDialog(QDialog):
    ...
    def buscar_por_clave(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, codigo_producto, nombre, descripcion, precio, stock FROM productos WHERE id = %s",
                (clave_producto,)
            )
            ...
            else:  QMessageBox.information(self, "Sin resultados", "No se encontró un producto con esa clave.")
        except mysql.connector.Error as e:
...
class CrearFacturaDialog(QDialog):
    ...
    def buscar_cliente(self):
        ...
        try:
            ...
            # Abrir diálogo para actualizar datos del cliente
                respuesta = QMessageBox.question(
                ...
                if respuesta == QMessageBox.Yes:
                    dialog = ActualizarDatosClienteDialog(cliente_id, self)
                    dialog.exec_()
                else:
                    self.cliente_label.setText(f"Cliente: {cliente[0]}")
                    self.cliente_id = cliente_id
...
class ListarFacturasDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Listar Facturas")
        self.setGeometry(200, 200, 800, 500)
        self.setStyleSheet("""
            QDialog {
                background-color: #FAF3E0;
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Cliente", "Vendedor", "Total"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.table.setSelectionBehavior(QTableWidget.SelectRows)
    ...
    def cargar_facturas(self):
        try:
            ...
            cursor = conn.cursor()
            query = """
                SELECT f.id, c.nombre AS cliente, v.username AS vendedor, f.total
                FROM facturas f
            ...
            QMessageBox.critical(self, "Error", f"Error al cargar facturas: {e}")
        finally:
            cursor.close()
            conn.close()
...
class ActualizarDatosClienteDialog(QDialog):
    def __init__(self, cliente_id, parent=None):
        super().__init__(parent)
        self.cliente_id = cliente_id
        self.setWindowTitle("Actualizar Datos del Cliente")
        self.setGeometry(300, 300, 400, 300)
        self.setStyleSheet("""
            QDialog {
                background-color: #F5F5DC;
    ...
    def cargar_datos_cliente(self):
        """Carga los datos actuales del cliente en los campos."""
        try:
            conn = conectar_db()
            cursor = conn.cursor()
            cursor.execute("SELECT nombre, telefono, direccion, correo FROM clientes WHERE id = %s", (self.cliente_id,))
            cliente = cursor.fetchone()
            ...
            QMessageBox.critical(self, "Error", f"Error al cargar datos del cliente: {e}")
        finally:
            cursor.close()
            conn.close()
    ...
    def actualizar_cliente(self):
        ...
        try:
            ...
            cursor.execute("""
                ...
                SET nombre = %s, telefono = %s, direccion = %s, correo = %s
                WHERE id = %s
            """, (nombre, telefono, direccion, email, self.cliente_id))
            conn.commit()
            QMessageBox.information(self, "Éxito", "Datos del cliente actualizados correctamente.")
            self.accept()  # Cierra el diálogo
        except mysql.connector.Error as e:
            QMessageBox.critical(self, "Error", f"Error al actualizar datos del cliente: {e}")
        finally:
            cursor.close()
            conn.close()
...
class Main(QMainWindow):
    def __init__(self):
        ...
        listar_facturas_action = QAction("Listar Facturas", self)

        facturacion_menu.addAction(crear_factura_action)
        facturacion_menu.addAction(listar_facturas_action)

        crear_factura_action.triggered.connect(self.crear_factura)
        listar_facturas_action.triggered.connect(self.listar_facturas)
    ...
    def listar_facturas(self):
        dialog = ListarFacturasDialog(self)
        dialog.exec_()
